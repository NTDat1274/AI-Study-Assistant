...
import { BookOpen, HelpCircle, MessageSquare, Send, Loader2, ArrowLeft, Play, CheckCircle2, XCircle } from 'lucide-react'
...
export default function StudyTabs({ 
  ...
  initialQuizzes: Quiz[],
  initialChatHistory: {role: string, content: string}[]
}) {
  // States for Summary
  ...
  // States for Chat
  const [chatHistory, setChatHistory] = useState<{role: string, content: string}[]>(initialChatHistory)
  ...
  const submitCurrentQuiz = async () => {
    ...
    if (res.success) {
      toast.success(`Đã nộp bài! Điểm: ${correctCount}/${activeQuiz.questions.length}`)
      // Update local list
      setQuizList(quizList.map(q => q.id === activeQuiz.id ? { ...q, score: correctCount, user_answers: userAnswers } : q))
    } else {
      ...
      <TabsContent value="summary">
        <Card>
          <CardHeader className="flex flex-row items-start justify-between">
            ...
            {!isLoadingSummary && <Button onClick={generateSummary} variant="outline" size="sm">
          ...
          <CardContent>
            {isLoadingSummary ? (
              ...
              <div className="text-center py-8">
                <p className="text-muted-foreground mb-4">Tài liệu chưa được tóm tắt.</p>
          ...
          <Card>
            <CardHeader className="flex flex-row items-start justify-between">
              <div>
                ...
                <CardDescription>Các bộ câu hỏi trắc nghiệm đã được AI tạo cho tài liệu này.</CardDescription>
              </div>
              <Button onClick={generateQuiz} disabled={isLoadingQuiz} variant="default">
            ...
            <CardContent>
              {quizList.length > 0 ? (
                  ...
                  {quizList.map((quiz, i) => (
                    <div key={quiz.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${quiz.score !== null ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600'}`}>
                          {quiz.score !== null ? <CheckCircle2 className="w-5 h-5" /> : <Play className="w-5 h-5 ml-1" />}
                        </div>
                        <div>
                          <p className="font-medium">Bài Quiz #{quizList.length - i}</p>
                          <p className="text-sm text-muted-foreground">{new Date(quiz.created_at).toLocaleString('vi-VN')}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        {quiz.score !== null && (
                          <div className="font-bold text-primary">Điểm: {quiz.score}/{quiz.questions.length}</div>
                        )}
                        <Button variant="outline" size="sm" onClick={() => openQuiz(quiz)}>
                ...
                <div className="text-center py-12 text-muted-foreground border border-dashed rounded-lg">
                  Chưa có bài quiz nào. Nhấn "Tạo bộ câu hỏi mới" để bắt đầu.
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center gap-4 pb-2">
              ...
              <div>
                ...
                <CardDescription>Chọn câu trả lời đúng nhất cho mỗi câu hỏi.</CardDescription>
              </div>
            </CardHeader>
            <CardContent>
              <div className="space-y-8 mt-4">
                ...
                {!showResults && (
                  <Button className="w-full mt-8" size="lg" onClick={submitCurrentQuiz}>Nộp bài kiểm tra</Button>
                )}
                {showResults && (
                  <div className="p-4 bg-primary/10 rounded-lg text-center mt-8">
                     <p className="text-xl font-bold text-primary">Điểm của bạn: {activeQuiz.score ?? 0} / {activeQuiz.questions.length}</p>
                     <Button className="mt-4" onClick={() => setActiveQuiz(null)}>Quay lại danh sách</Button>
                  </div>
                )}
              </div>
            </CardContent>
