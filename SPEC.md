...
- **Dữ liệu đầu vào:** Text của tài liệu, mức độ khó (Dễ/Trung bình/Khó - _Optionally_).
...
## 7. Module Quản trị Hệ thống (Admin Dashboard)

Hệ thống cung cấp một phân hệ quản trị toàn diện để giám sát hoạt động và tài nguyên.

### 7.1. Phân quyền và Bảo mật (Security & RBAC)

- Bổ sung thuộc tính `role` (giá trị: 'user' hoặc 'admin') vào bảng `users`.
- Thiết lập Next.js Middleware để kiểm tra `role` và bảo vệ các đường dẫn `/admin/*`.

### 7.2. Các tính năng quản trị chính

1. **Tổng quan (Overview):**
   - Thẻ thống kê (Cards): Tổng số người dùng, tổng số tài liệu, tổng số quiz đã tạo, tổng số request AI.
   - Biểu đồ (Charts): Trực quan hóa tần suất sử dụng hệ thống trong 7 ngày gần nhất.
2. **Quản lý Người dùng (Users):**
   - Hiển thị danh sách người dùng (ID, Email, Tên, Ngày tham gia, Role).
   - Có thể mở rộng chức năng Khóa/Xóa tài khoản trong tương lai.
3. **Quản lý Tài liệu (Documents):**
   - Hiển thị toàn bộ tài liệu đã được tải lên hệ thống (Tên file, Dung lượng, Định dạng, Chủ sở hữu).
   - Cho phép Admin xóa các tài liệu rác hoặc vi phạm chính sách để tiết kiệm Storage.
4. **Quản lý Quiz (Quizzes):**
   - Bảng thống kê các bài trắc nghiệm người dùng đã tạo và lưu trữ.
5. **Thống kê AI Usage (Gemini API):**
   - Tạo bảng `api_logs` trong Database để ghi nhận chi tiết mỗi khi tính năng AI được gọi (Tóm tắt, Quiz, Chat).
   - Phân tích dữ liệu log để xem tính năng nào được sử dụng nhiều nhất, giúp quản trị viên ước lượng chi phí API hiệu quả.
